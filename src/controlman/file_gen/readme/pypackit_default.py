...
class PyPackITDefaultReadmeFileGenerator(ReadmeFileGenerator):
    def __init__(
        self,
        data: ControlCenterContentManager,
        path_manager: PathManager,
        custom_generator: _ControlCenterCustomContentGenerator,
        target: Literal["github", "pypi", "conda"],
    ):
        super().__init__(data=data, path_manager=path_manager, custom_generator=custom_generator, target=target)
        self._data = self._data["readme"][target]["config"]
    ...
    def generate(self) -> list[tuple[DynamicFile, str]]:
        ...
        html_content = html.ElementCollection(
            elements=[
                html.Comment(f"{self._data['name']} ReadMe File"),
    ...
    def logo(self) -> html.A:
        ...
        url = (
            f"{self._data['path']['dir']['control']}/ui/branding/logo_full_{{}}.svg" if self._supports_dark
            else f"{self._data['url']['website']['home']}/_static/logo_full_{{}}.svg"
        )
        img = html.img(
        ...
        logo = html.A(href=self._data["url"]["website"]["home"], content=[tag])
        if style == "horizontal":
            logo.content.elements.append(self.spacer(width="10px", height=data["height"], align="left"))
        return logo

    def header_body(self):
        description = html.P(align="justify", content=[self._data["description"]]).style(
            {
                self._data["name"]: {
                    "bold": True,
                    "italic": True,
                    "link": self._data["url"]["website"]["home"],
                }
            }
        )
        return description
    ...
    def body_keynotes(self, config: dict):
        content = []
        for key_point in self._data["keynotes"]:
            content.extend(
                [
                    ...
                    self.button(
                        ...
                        color_light=self._data["theme"]["color"]["primary"][0],
                        color_dark=self._data["theme"]["color"]["primary"][1],
                    ),
    ...
    def menu(self):
        top_data = self._data["web"]["sections"]
        bottom_data = self._data["header"]["menu_bottom"]["buttons"]
        colors_light, colors_dark = [
            pcit.gradient.interpolate_rgb(
                color_start=pcit.color.hexa(self._data["theme"]["color"]["primary"][theme]),
                color_end=pcit.color.hexa(self._data["theme"]["color"]["secondary"][theme]),
        ...
        buttons = [
            self.button(
                ...
                link=f"{self._data['url']['website']['home']}/{data['path']}",
            )
    ...
    def spacer(self, **args):
        spacer = html.IMG(
            src=f"{self._data['url']['website']['home']}/_static/img/spacer.svg",
            **args,
        )
        return spacer
